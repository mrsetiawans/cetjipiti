// --- VALIDATION LOGIC ---
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const getFeaturesHTML = (pkg, colorOffset = 0) => {
            ...
            return `<div class="material-card-elevated p-4 md:p-6 rounded-2xl">
                ...
                <ul class="space-y-3">${pkg.features.map((f, index) => `<li class="flex items-start"><i class="fas fa-check-circle ${googleColors[(index + colorOffset) % googleColors.length]} mt-1 mr-3 text-lg feature-icon"></i><span class="text-theme-on-surface-variant text-sm leading-relaxed">${f}</span></li>`).join('')}</ul>
            </div>`;
        }
        ...
        function updateDisplay() {
            ...
            setTimeout(() => {
                ...
                const colorOffset = pkg.id === 'photos' ? 2 : 0; // Start drive with blue, photos with yellow
                infoContainer.innerHTML = getProductInfoHTML(pkg);
                featuresContainer.innerHTML = getFeaturesHTML(pkg, colorOffset);
